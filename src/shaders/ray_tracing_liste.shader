...
vec3 Radiance(Ray input_ray, Sphere list[MAX_SIZE], int size)
{
  ...
  for(int i=0; i<REFLECTION_NUMBER; ++i)
  {
    ...
    Ray refracted_ray=input_ray;
    // POOF we touch an object
    
    if(refracted.material.transparency > 0.0f)
    {
      for(int j=0; j<3; j++)
      {
        ...
        is_inside=dot(refracted_ray.direction, refracted.normal) < 0;

        if(is_inside)
          refraction_ratio = old_refraction_index / refracted.material.n;
        else
          refraction_ratio=refracted.material.n;
          
        float cos_t = min(dot(-refracted_ray.direction, refracted.normal),1.0);
        float sin_t = sqrt(1.0 - cos_t * cos_t);

        bool cannot_refract = refracted.material.n * sin_t > 1.0;
        vec3 direction;
        if(cannot_refract)
        {
          direction=reflect(refracted_ray.direction, refracted.normal);
          break;
        }
        else
        {
          old_refraction_index=refracted.material.n;

          direction=refract(refracted_ray.direction, refracted.normal, refraction_ratio);
          
          refracted = ClosestHitPoint(Ray(refracted_ray.origin + (refracted.distance + epsilon) * refracted_ray.direction, direction), list, size);
          color +=  refracted.material.color * refracted.material.transparency;
          
          refracted_ray.direction=direction;
          refracted_ray.origin=refracted_ray.origin + (refracted.distance+epsilon) * refracted_ray.direction * attenuated_color;
          attenuated_color*=0.8;
        }
      }
    }
    ...
    else if(intersection.material.diffuse <= 0.0f && intersection.material.shininess <= 0.0f && intersection.material.transparency <=0.0f)
    {
...
struct RayStack
{
  Ray ray_list[REFLECTION_NUMBER];
  int size;

};
RayStack CreateRayStack()
{
  RayStack stack;
  stack.size=0;
  return stack;
}
Ray Dequeue(inout RayStack rs)
{
  return rs.ray_list[--rs.size];
}

void Enqueue(inout RayStack rs, Ray ray)
{
  rs.ray_list[rs.size++]= ray;
}
bool is_empty(const RayStack rs)
{
  return (rs.size==0);
}
...
vec3 Radiance(Ray input_ray, Sphere list[MAX_SIZE], int size)
{
  ...
  // List of reflected rays to manage at step N. (same level of "recursion")
  RayStack reflection_stack = CreateRayStack();
  RayStack waiting_stack=reflection_stack;

  Enqueue(reflection_stack, input_ray);

  for(int i=0; i<MAX_DEPTH; ++i) // For each recursion level
  { 
    while(!is_empty(reflection_stack)) // Compute all the rays
    {
      input_ray=Dequeue(reflection_stack);
      Hit intersection=ClosestHitPoint(input_ray, list, size);
      // POOF we touch an object
      if(intersection.material.diffuse > 0.0f || intersection.material.shininess > 0.0f)
      {
        ...
        // We reflect the initial ray
        vec3 reflection = reflect(input_ray.direction, intersection.normal);
        Enqueue(waiting_stack,Ray(input_ray.origin + intersection.distance * input_ray.direction + epsilon * reflection, reflection));
      }
      ...
      else if(intersection.material.diffuse <= 0.0f && intersection.material.shininess <= 0.0f)
      {
        vec3 spotlight = vec3(1e6) * pow(abs(dot(input_ray.direction, light.direction)), 250.0);
        color += mask * (ambient + spotlight); 
        return color; // It is useless to iterate any longer
      }
    }
    reflection_stack=waiting_stack;
    waiting_stack.size=0;
  }
  return color;
}
