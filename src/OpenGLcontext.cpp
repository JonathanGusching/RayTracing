...
void OpenGLcontext::CreateRenderProgramAndShaders()
{
    ...
    {
        std::cerr << "Error: could not create render program" << std::endl;
        return;
    }
    ...
    for(GLuint i=0; i<shaders.size(); i++)
    {
        ...
        if(shaders[i]->type!=GL_COMPUTE_SHADER)
        {
            ...
            switch(error)
            {
                case(GL_INVALID_VALUE):std::cerr<< "ERROR: Program or shader isn't a value generated by OpenGL" << std::endl; break;
                case(GL_INVALID_OPERATION):std::cerr<< "ERROR: Program or shader is not a program/shader object" << std::endl; break;
...
void OpenGLcontext::CreateComputeProgram()
{
    ...
    {
        std::cerr << "Error: could not create compute program" << std::endl;
        return;
    }
...
const void OpenGLcontext::Render()
{
    ...
    if(glGetError()!=0)
    {
        std::cerr << "OpenGL RENDERING ERROR ====== \n";
    }
